...
def test_health_returns_ok(hub_client: TestClient) -> None:
...
def test_list_agents_and_get_agent_return_expected_shape(hub_client: TestClient) -> None:
    import app.main as main_module

    state = main_module.hub_state
    attach_agent(state, "agent-a")

    response = hub_client.get("/api/agents", headers={"X-Admin-Token": "test-admin-token"})
    ...
    agent_response = hub_client.get("/api/agents/agent-a", headers={"X-Admin-Token": "test-admin-token"})

    assert agent_response.status_code == 200
    assert agent_response.json()["agentId"] == "agent-a"


def test_register_frame_exposes_capabilities_in_agent_snapshot(hub_client: TestClient) -> None:
    ...
    response = hub_client.get("/api/agents/agent-a", headers={"X-Admin-Token": "test-admin-token"})
...
def test_disconnect_clears_agent_runtime_capabilities(hub_client: TestClient) -> None:
...
def test_agent_status_includes_queued_and_processing_command_summary(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_rotate_agent_credentials_requires_admin_token_and_persists_state(hub_client: TestClient) -> None:
    forbidden = hub_client.post("/api/agents/agent-a/credentials/rotate")
...
def test_provision_agent_creates_offline_agent_and_returns_initial_key(hub_client: TestClient) -> None:
    forbidden = hub_client.post("/api/agents", json={"agentId": "agent-new"})
...
def test_provisioned_key_remains_valid_even_if_issued_at_is_old(hub_client: TestClient) -> None:
    import app.main as main_module
    ...
    with hub_client.websocket_connect(f"/ws/agent/agent-e2e?key={agent_key}") as websocket:
        websocket.send_json({"type": "heartbeat"})

    agent_response = hub_client.get("/api/agents/agent-e2e", headers={"X-Admin-Token": "test-admin-token"})
    assert agent_response.status_code == 200
    assert agent_response.json()["credentialConfigured"] is True


def test_get_unknown_agent_returns_404(hub_client: TestClient) -> None:
    response = hub_client.get("/api/agents/missing-agent", headers={"X-Admin-Token": "test-admin-token"})

    assert response.status_code == 404
    assert response.json() == {"detail": "Agent not found"}


def test_dispatch_command_creates_record_and_returns_expected_payload(hub_client: TestClient) -> None:
    import app.main as main_module
    ...
    get_response = hub_client.get("/api/commands/req-dispatch-1", headers={"X-Admin-Token": "test-admin-token"})
    assert get_response.status_code == 200
    assert get_response.json()["requestId"] == "req-dispatch-1"


def test_dispatch_graceful_stop_forwards_drain_params_to_agent(hub_client: TestClient) -> None:
...
def test_dispatch_derives_requested_by_from_admin_token_ignoring_client_header(
    ...
    get_response = hub_client.get("/api/commands/req-derive-1", headers={"X-Admin-Token": "test-admin-token"})
    assert get_response.status_code == 200
...
def test_retry_derives_requested_by_from_admin_token_ignoring_client_header(hub_client: TestClient) -> None:
    ...
    get_response = hub_client.get(f"/api/commands/{new_request_id}", headers={"X-Admin-Token": "test-admin-token"})
    assert get_response.status_code == 200
    assert get_response.json()["requestedBy"] == "platform-admin"


def test_dispatch_requires_admin_token(hub_client: TestClient) -> None:
    response = hub_client.post(
        "/api/agents/agent-a/commands",
        json={
...
def test_dispatch_update_without_image_returns_422(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_dispatch_stop_with_mode_persists_and_returns_202(hub_client: TestClient) -> None:
    ...
    get_response = hub_client.get("/api/commands/req-stop-graceful", headers={"X-Admin-Token": "test-admin-token"})
    assert get_response.status_code == 200
    assert get_response.json()["mode"] == "graceful"


def test_dispatch_accepts_new_actions(hub_client: TestClient) -> None:
...
def test_dispatch_pull_redeploy_without_image_returns_422(hub_client: TestClient) -> None:
...
def test_dispatch_restart_without_mode_persists_null_mode(hub_client: TestClient) -> None:
...
def test_dispatch_command_to_offline_agent_returns_409(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_get_command_events_returns_created_ack_and_result(hub_client: TestClient) -> None:
    import app.main as main_module
    ...
    response = hub_client.get("/api/commands/req-events-1/events", headers={"X-Admin-Token": "test-admin-token"})
...
def test_get_unknown_command_and_events_return_404(hub_client: TestClient) -> None:
    command_response = hub_client.get("/api/commands/missing", headers={"X-Admin-Token": "test-admin-token"})
    events_response = hub_client.get("/api/commands/missing/events", headers={"X-Admin-Token": "test-admin-token"})
...
def test_list_commands_supports_sort_and_pagination(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_list_commands_tolerates_empty_query_values(hub_client: TestClient) -> None:
    response = hub_client.get(
        "/api/commands",
        params={
            "agentId": "",
...
def test_list_commands_supports_agent_filter(hub_client: TestClient) -> None:
    import app.main as main_module
    ...
    filtered_response = hub_client.get("/api/commands", params={"agentId": "agent-a"}, headers={"X-Admin-Token": "test-admin-token"})
...
def test_list_commands_tolerates_empty_query_values_after_agent_filter_cleanup(hub_client: TestClient) -> None:
    response = hub_client.get(
        "/api/commands",
        params={
            "agentId": "",
...
def test_command_history_is_paginated_with_agent_filter(hub_client: TestClient) -> None:
    import app.main as main_module
    state = main_module.hub_state
    ...
    response = hub_client.get("/api/commands", params={"agentId": "agent-a", "status": "failed"}, headers={"X-Admin-Token": "test-admin-token"})
...
def test_retry_failed_command_creates_new_command_and_audit_event(hub_client: TestClient) -> None:
    import app.main as main_module
    state = main_module.hub_state
    fake_socket = attach_agent(state, "agent-a")
    asyncio.run(state.store_command("agent-a", {"type": "command", "requestId": "req-1", "action": "restart", "dir": "/srv/a"}, requested_by="platform-api", request_source="ops-console"))
    ...
    events_response = hub_client.get("/api/commands/req-1/events", headers={"X-Admin-Token": "test-admin-token"})
    assert events_response.status_code == 200
    event_types = [item["eventType"] for item in events_response.json()]
    assert "retry" in event_types


def test_retry_non_failed_command_returns_409(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_retry_missing_command_returns_404(hub_client: TestClient) -> None:
    response = hub_client.post(
...
def test_stream_agent_logs_returns_sse_events(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_stream_agent_logs_returns_stream_error_event(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_rolling_router_mounted(hub_client: TestClient) -> None:
    # 未带 token 返回 403 而非 404,证明路由已注册
    resp = hub_client.post("/api/rolling-restart", json={"agentId": "a", "serviceName": "s"})
    assert resp.status_code == 403


def test_rolling_restart_returns_task_id(hub_client: TestClient) -> None:
...
def test_rolling_status_404_unknown(hub_client: TestClient) -> None:
    resp = hub_client.get("/api/rolling-restart/nope", headers={"X-Admin-Token": "test-admin-token"})
...
def test_rolling_acknowledge_requires_admin_token(hub_client: TestClient) -> None:
    # 不带 token → 403(鉴权在端点首行)
    resp = hub_client.post("/api/rolling-restart/whatever/acknowledge")
    assert resp.status_code == 403
    assert resp.json() == {"detail": "Invalid admin token"}


def test_rolling_acknowledge_unknown_task_returns_404(hub_client: TestClient) -> None:
    resp = hub_client.post(
...
def test_rolling_acknowledge_releases_lock_end_to_end(hub_client: TestClient) -> None:
...
def test_rolling_acknowledge_non_interrupted_returns_409(hub_client: TestClient) -> None:
...
def test_log_stream_subscriptions_share_upstream_and_stop_on_last_subscriber(hub_client: TestClient) -> None:
    import app.main as main_module
...
def test_list_instances_requires_admin_token(hub_client: TestClient) -> None:
...
def test_list_instances_returns_instances_for_online_agent(hub_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_list_instances_forwards_expected_compose_project(hub_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_list_instances_to_offline_agent_returns_409(hub_client: TestClient) -> None:
...
def test_list_instances_to_unknown_agent_returns_404(hub_client: TestClient) -> None:
    # agent 不存在 → 404。
    response = hub_client.post(
...
def test_list_instances_agent_failure_returns_502(hub_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_list_instances_agent_timeout_returns_502(hub_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_list_instances_agent_connection_lost_returns_502(hub_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
