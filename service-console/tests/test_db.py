...
from app.hub.db_models import CommandModel
from app.hub.store import utc_now
...
def test_init_schema_stamps_fully_initialized_legacy_schema(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    # 合并后(S4)单一 DB 共 12 张被管表(console 8 + hub 4);"已完整建表但无 alembic_version"
    # 的库须被 stamp 到 head。Inspector 须返回全部 12 张表,否则会落到「partial legacy」拒绝分支。
    database = Database(f"sqlite:///{tmp_path / 'legacy.db'}")
    stamp_calls: list[tuple[object, str]] = []
    upgrade_calls: list[tuple[object, str]] = []

    fully_initialized = sorted(Database._managed_tables)
