...
| **P3-5 ✓(表/迁移/读端点)** | **DiscoveredNode 表 + 迁移 + 读端点**:`db_models.DiscoveredNodeModel`(uq agent_id+container_name)+ 迁移 `7f3a9c2e1b04`+ `_managed_tables` 13 张;读端点 `GET /api/nodes/instances`(分页 + namespace/status 过滤,默认含 stale 可见,`DiscoveredNodeOut` camelCase,require_session)供实例页消费。dir/镜像权威在此(P0-4)。**实例页 SPA UI 归入前端批次**(随 P3-7~P3-10) | service-console `db_models.py`/迁移/`nodes.py`/`models.py`(✓);SPA 实例页(待) | P3-4 | **表/迁移/读端点已完成**(409 测试绿,95.15%);UI 待前端批次 |
