...
**P1 验收(rolltest 床)✅(2026-06-24)**:openclaw 床(192.168.31.57)上用当前 main 重建 agent(`rolltest-agent:local`,因床无 docker hub 直连,以现有镜像为 base 叠加源码)+ 复用合并版 console;造最小发布链(ns `rolltest-agent` / service `p1demo` / `@test/plugin-p1-demo@1.0.0`)。`docker exec` 在 agent netns 内模拟 worker(127.0.0.1:18082):① 列清单 `url` 正确改写指向本机 `/download/1`;② 首次 `/download/1` **回源**(agent `/health` cache `count 0→1`、console `fetch_record` 1 条 + nginx download 访问行恰好 1 行);③ 二次 `/download/1` **命中**(缓存不变、console 无新请求);两次下载 sha256 == 源包,10/10 断言全绿。**真 worker 经 `sync-plugins` 拉取属 P2**(需把 worker 配置指向本机 agent,cnp + 部署侧)。
...
| **P3-4 ✓** | **console 接收发现上报 + 落库 + 心跳**:`api_support._handle_agent_message` 加 `discovery-report` 分支 → `hub_state.record_discovery`(`_record_discovery_sync` 走 `asyncio.to_thread`);按 (agent_id, container_name) upsert 为 active + 刷 heartbeat_at,**本轮缺席的同 agent 行标 `stale` 不删**(评审 M8;空上报=该 agent 全行 stale,显式 where 避 notin_([]));落位 warnings 只记日志 | service-console `store.py`/`api_support.py` | M, P3-3 | **已完成**(386 测试绿,94.92%);agent 失联节点保留可定位 |
| **P3-5 ✓** | **DiscoveredNode 表 + 迁移 + 读端点 + 实例页 UI**:`DiscoveredNodeModel`(uq agent_id+container_name)+ 迁移 `7f3a9c2e1b04`+ `_managed_tables` 13 张;读端点 `GET /api/nodes/instances`(分页 + namespace/status 过滤,默认含 stale 可见,`DiscoveredNodeOut` camelCase);**SPA 实例页 `InstancesPage`**(ProTable 只读列表,贴原型列/徽章:命名空间/服务/容器/工程/目录/镜像/运行/健康/状态/心跳,namespace+status 筛选 + 服务端分页,发现权威 dir/image/composeProject);路由 `/instances` + 侧栏菜单。dir/镜像权威在此(P0-4)。**per-instance 操作/日志查看器归后续(P3-9 等)** | service-console 后端(✓)+ web `InstancesPage`(✓) | P3-4 | **已完成**(后端 409 测试 95.15%;前端 67 vitest 绿、lint/build 过、InstancesPage 97.6%);实例页只读可用 |
| **P3-6 ✓** | **node-operations 寻址迁到 DiscoveredNode**(评审 H-3,发现权威):`nodes.py` 新增 `_resolve_addressing`(`_NodeAddressing`):入口 `_resolve_service` 取 Service 行(nacosService 入口 + 回退值),按 (agentId, nacosService) 查 active DiscoveredNode —— **0 个→回退 Service.dir/default_image(迁移期);1 个→DiscoveredNode 权威;多实例→按 composeProject 选(缺则 409、未匹配 404)**。dir/image/containerId **优先 DiscoveredNode、Service 仅回退**;expectedComposeProject 优先 DiscoveredNode 真实 compose_project。`NodeActionIn` 加 composeProject;degraded 不变式/camelCase/requested_by 不变。list_nodes 展示:唯一发现行→显示发现值,多实例→暂显 Service 值(per-instance 列表归实例页) | service-console `nodes.py`/`store.py`/`models.py` | P3-5 | **已完成**(404 测试绿,95.03%,nodes.py 100%);Service.dir 空但 DiscoveredNode 有 dir 仍可下发 |
