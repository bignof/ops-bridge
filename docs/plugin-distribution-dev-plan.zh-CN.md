...
| **P3-5 ✓(表/迁移)** | **DiscoveredNode 表 + 迁移**:`db_models.DiscoveredNodeModel`(uq agent_id+container_name)+ 迁移 `7f3a9c2e1b04`(down_rev 682a89c2f7d1)+ `_managed_tables` 13 张;全自动 upsert;dir/镜像权威在此(P0-4)。**实例页 SPA UI 归入前端批次**(随 P3-7~P3-10) | service-console `db_models.py`+迁移(✓);SPA 实例页(待) | P3-4 | **表/迁移已完成**(fresh sqlite upgrade 自检过);UI 待前端批次 |
