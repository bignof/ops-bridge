...
@app.post("/api/agents", response_model=AgentProvisionResponse, status_code=status.HTTP_201_CREATED, summary="创建 Agent 并签发初始密钥", description="创建新的 Agent 记录，并返回该 Agent 的首次连接密钥。密钥默认长期有效，直到显式轮换。", tags=["Agent 管理"])
...
@app.post("/api/agents/{agent_id}/credentials/rotate", response_model=AgentCredentialResponse, summary="轮换 Agent 密钥", description="为指定 Agent 重新签发连接密钥。新密钥默认长期有效，旧密钥在轮换后立即失效。", tags=["Agent 管理"])
